...
const Navbar = () => {
  ...
  const handleSignOut = async () => {
    await signOut(); // AWS amplify signOut API call to signout
    window.location.href = "/"; // used this instead of Router so that page refreshes after logout
  };
          ...
          {authUser ? (
            <>
              <div className="relative hidden md:block" >
                ...
                <div className="relative hidden md:block" >
                <Bell className="w06 h-6 cursor-pointer text-primary-200 hover:text-primary-400" />
                <span className="absolute top-0 right-0 w-2 h-2 bg-secondary-700 rounded-full" ></span>
              </div>
              <DropdownMenu>
